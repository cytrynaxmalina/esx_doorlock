...
Citizen.CreateThread(function()
	while true do
		...
		if Current then
			...
			if door.draw then
				DrawText3Ds(door.position.x, door.position.y, door.position.z, (door.gate == nil and (door.locked and '[E] ~r~Zamknięte' or '[E] ~g~Otwarte') or (door.locked and '[H] ~r~Zamknięte' or '[H] ~g~Otwarte')))
			end
			
			if IsControlJustReleased(1, (door.gate == nil and 51 or 74)) and door.locked ~= nil and PlayerData and PlayerData.job then
				local let = true
				if door.jobs then
					let = door.jobs[PlayerData.job.name] == true 
				end
				...
				if let then
					...
					if door.soundLib then
						if not door.locked then							
							TriggerServerEvent('InteractSound_SV:PlayOnSource','unlockDoor', 0.5)
						else
							TriggerServerEvent('InteractSound_SV:PlayOnSource','lockDoor', 0.5)
						end
					else
						if not door.locked then
							TriggerServerEvent('InteractSound_SV:PlayOnSource','unlockDoor', 0.5)
						else
							TriggerServerEvent('InteractSound_SV:PlayOnSource','lockDoor', 0.5)						end
					end
